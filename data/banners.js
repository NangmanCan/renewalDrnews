export const initialBanners = [
  {
    id: 1,
    title: '의료기기 박람회 2026',
    description: '국내 최대 의료기기 전시회',
    image: 'https://images.unsplash.com/photo-1551076805-e1869033e561?w=800&h=400&fit=crop',
    link: '#',
    type: 'headline', // headline, bottom, sidebar
    isActive: true,
    order: 1,
  },
  {
    id: 2,
    title: '대한병원협회 신규 회원 모집',
    description: '함께 성장하는 의료 네트워크',
    image: 'https://images.unsplash.com/photo-1538108149393-fbbd81895907?w=800&h=400&fit=crop',
    link: '#',
    type: 'headline',
    isActive: true,
    order: 2,
  },
  {
    id: 3,
    title: '의료 AI 솔루션 - MedTech',
    description: '진단 정확도 95% 달성',
    image: 'https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?w=800&h=400&fit=crop',
    link: '#',
    type: 'bottom',
    isActive: true,
    order: 1,
  },
  {
    id: 4,
    title: '제약사 채용 공고',
    description: '글로벌 제약사 한국지사 채용',
    image: 'https://images.unsplash.com/photo-1587854692152-cbe660dbde88?w=800&h=400&fit=crop',
    link: '#',
    type: 'bottom',
    isActive: true,
    order: 2,
  },
  {
    id: 5,
    title: '의료 세미나 안내',
    description: '2026 디지털 헬스케어 컨퍼런스',
    image: 'https://images.unsplash.com/photo-1576091160550-2173dba999ef?w=800&h=400&fit=crop',
    link: '#',
    type: 'bottom',
    isActive: false,
    order: 3,
  },
  {
    id: 6,
    title: '병원 경영 컨설팅',
    description: '전문가와 함께하는 병원 성장 전략',
    image: 'https://images.unsplash.com/photo-1579154204601-01588f351e67?w=800&h=400&fit=crop',
    link: '#',
    type: 'sidebar',
    isActive: true,
    order: 1,
  },
  {
    id: 7,
    title: '의료 법률 컨설팅 서비스',
    description: '의료분쟁·계약 전문 법률 지원',
    image: 'https://images.unsplash.com/photo-1554260570-9140fd3b7614?w=800&h=400&fit=crop',
    link: '#',
    type: 'headline',
    isActive: true,
    order: 3,
  },
  {
    id: 8,
    title: '의사 전용 보험 가입',
    description: '전문의 맞춤형 종합보험',
    image: 'https://images.unsplash.com/photo-1491795644452-40947ca5db58?w=800&h=400&fit=crop',
    link: '#',
    type: 'headline',
    isActive: true,
    order: 4,
  },
  {
    id: 9,
    title: '전문학술지 구독 안내',
    description: '최신 의학 연구 논문을 한눈에',
    image: 'https://images.unsplash.com/photo-1532049364073-77cbf45d7505?w=800&h=400&fit=crop',
    link: '#',
    type: 'headline',
    isActive: true,
    order: 5,
  },
];
