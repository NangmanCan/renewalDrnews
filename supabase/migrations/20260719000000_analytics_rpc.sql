...
-- 모든 함수는 기간 범위(start_ts, end_ts) 기반으로 통일: created_at >= start_ts AND created_at < end_ts

-- 구버전 시그니처 제거 (로컬에 구버전이 적용돼 있어 같은 이름·다른 시그니처 중복 방지)
DROP FUNCTION IF EXISTS analytics_unique_visitors(TIMESTAMPTZ);
DROP FUNCTION IF EXISTS analytics_daily_series(INTEGER);
DROP FUNCTION IF EXISTS analytics_top_articles(TIMESTAMPTZ, INTEGER);
DROP FUNCTION IF EXISTS analytics_referrer_stats(TIMESTAMPTZ);

-- 1) 순 방문자 수 — 기존 JS Set 방식의 1000행 과소집계 수정 (기간 범위 기반)
...
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  ...
  FROM page_views
  WHERE created_at >= start_ts AND created_at < end_ts AND visitor_id IS NOT NULL;
$$;

-- 2) 일별 PV/UV 추이 (KST 기준 일 단위, 기간 범위 기반)
...
-- 3) 기간별 인기 기사 (/article/{id} 경로 집계, 기간 범위 기반)
...
LANGUAGE sql STABLE SECURITY DEFINER AS $$
  SELECT (regexp_match(page_path, '^/article/(\d+)'))[1]::bigint AS article_id,
         COUNT(*) AS view_count
  FROM page_views
  WHERE created_at >= start_ts AND created_at < end_ts AND page_path ~ '^/article/\d+'
  GROUP BY 1
  ORDER BY 2 DESC
  LIMIT lim;
$$;

-- 4) 유입 채널 집계 (검색 등록 효과 측정용, 기간 범위 기반)
