...
// 본문 사진 정렬 컨트롤바 버튼 (BubbleMenu 내부)
const FigureMenuButton = ({ onClick, active, danger, children, title }) => (
  <button
    type="button"
    onClick={onClick}
    title={title}
...
export default function TipTapEditor({ content, onChange, placeholder = '본문을 입력하세요...', enableInlinePhoto = false }) {
  ...
  // 본문 사진(articleFigure) 정렬/폭 변경 — 에디터 내 컨트롤바에서 호출
  // 상호배제 규칙: 전체폭 선택 시 align 중앙, 좌/우 선택 시 full 해제
  const setFigureAlign = useCallback(
    (align) => {
      ...
      editor
        .chain()
        .focus()
        .updateAttributes('articleFigure', { align, full: false })
        .run();
    },
    [editor]
  );

  const setFigureFull = useCallback(() => {
    if (!editor) return;
    editor
      .chain()
      .focus()
      .updateAttributes('articleFigure', { full: true, align: 'center' })
      .run();
  }, [editor]);

  const setFigureMedium = useCallback(() => {
    if (!editor) return;
    editor.chain().focus().updateAttributes('articleFigure', { full: false }).run();
  }, [editor]);
  ...
  return (
    <div className="border border-gray-300 rounded-lg bg-white">
      ...
      <div className="border-b border-gray-300 bg-gray-50 p-2 flex flex-wrap gap-1 items-center relative z-20">
        ...
        {/* 이미지 삽입 — 기사 에디터(enableInlinePhoto)에서는 본문 사진 모달로 통합 */}
        <ToolbarButton
          onClick={() =>
            enableInlinePhoto ? setInlinePhotoOpen(true) : imageInputRef.current?.click()
          }
          disabled={uploading}
          title={enableInlinePhoto ? '본문 사진 추가 (캡션·출처·워터마크)' : '이미지 삽입'}
          ariaLabel="이미지 삽입"
        >
          {uploading ? (
      ...
      {/* 본문 사진 정렬 컨트롤 (articleFigure 선택 시에만 노출) */}
      <BubbleMenu
        editor={editor}
        pluginKey="articleFigureMenu"
        shouldShow={({ editor: ed }) => ed.isActive('articleFigure')}
        options={{ placement: 'top' }}
        className="flex items-center gap-0.5 bg-white border border-gray-300 rounded-lg shadow-lg px-1 py-1"
      >
        <FigureMenuButton
          onClick={() => setFigureAlign('left')}
          active={editor.isActive('articleFigure', { align: 'left', full: false })}
          title="왼쪽 정렬"
        >
          ◀ 왼쪽
        </FigureMenuButton>
        <FigureMenuButton
          ...
          active={editor.isActive('articleFigure', { align: 'center', full: false })}
          title="중앙 정렬"
        >
          ■ 중앙
        </FigureMenuButton>
        <FigureMenuButton
          ...
          active={editor.isActive('articleFigure', { align: 'right', full: false })}
          title="오른쪽 정렬"
        >
          ▶ 오른쪽
        </FigureMenuButton>
        ...
        <FigureMenuButton
          onClick={setFigureMedium}
          active={editor.isActive('articleFigure', { full: false })}
          title="중간 폭"
        >
          중간
        </FigureMenuButton>
        <FigureMenuButton
          onClick={setFigureFull}
          active={editor.isActive('articleFigure', { full: true })}
          title="전체폭"
        >
          전체폭
        </FigureMenuButton>
        <div className="w-px h-5 bg-gray-300 mx-0.5" />
        <FigureMenuButton onClick={deleteFigure} title="삭제" danger>
          🗑 삭제
        </FigureMenuButton>
      </BubbleMenu>
