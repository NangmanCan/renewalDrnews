...
function PickItem({ label, title, href }) {
  return (
    ...
    >
      <div className="text-xs font-bold text-brand-600 mb-1 tracking-wide">{label}</div>
      <div className="text-[15px] font-bold text-navy leading-snug line-clamp-1 group-hover:text-brand-600 transition-colors">
        {title}
      </div>
    </Link>
  );
    ...
    <>
      ...
      <div className="hidden lg:block bg-white">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-stretch gap-6 border-b-4 border-brand-600">
          ...
          <div className="flex-shrink-0 w-36 pl-5 flex flex-col justify-center">
            <div className="text-sm font-extrabold text-brand-600 tracking-wider">DOCTOR&apos;S PICK</div>
            <div className="text-xs text-gray-500 mt-1">{formatKoreanDate()}</div>
          </div>
