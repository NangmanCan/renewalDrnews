...
// Intl 포매터는 생성 비용이 커서(건당 ~0.04ms) 모듈 레벨에서 1회만 생성해 재사용
// (전체 기사 포맷 시 엣지 워커 CPU 제한 초과 방지 — Error 1102)
...
function formatArticle(item) {
  return {
    ...item,
    // KST 기준 게재일 (toISOString은 UTC라 오전 9시 이전 발행 기사가 전날로 표시됨)
    date: item.created_at ? kstDateFormatter.format(new Date(item.created_at)) : null,
    publishedAt: item.created_at || null, // 풀 ISO 타임스탬프 (구조화 데이터/RSS용)
    modifiedAt: item.updated_at || null, // 수정 시각 (updated_at 컬럼 있을 때만)
...
// 최신 기사 N건 (본문 제외 경량 조회 — 사이드바용. 전체 조회는 엣지 CPU 제한 초과 위험)
export async function getLatestArticles(limit = 8) {
  ...
  try {
    const { data, error } = await supabase
      .from('articles')
      .select('id, title, category, image, author, views, created_at, is_headline, placement')
      .order('created_at', { ascending: false })
      .limit(limit);

    if (error) throw error;
    const formatted = formatArticles(data);
    return formatted.length > 0 ? formatted : staticArticles.slice(0, limit);
  } catch (error) {
...
export async function getPopularArticles(limit = 5) {
  ...
  try {
    // 최근 60일 기사만 대상 (오래된 기사가 누적 조회수로 상위 고정되는 것 방지)
...
// 기자별 기사 조회 (기자 프로필 페이지용, 부분 일치)
export async function getArticlesByAuthor(authorName, limit = 20) {
  if (!supabase) {
    return staticArticles
      .filter((article) => article.author?.includes(authorName))
      .slice(0, limit);
  }
