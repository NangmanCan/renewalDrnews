...
import { fetchHiraPress } from './scrapers/hiraPress';
...
// 텍스트 값 추출 (문자열 / 객체 / 배열 / 중첩 HTML 태그 대응)
function extractText(value) {
  ...
  if (typeof value === 'object') {
    ...
    // 예: <title><a href="...">제목</a></title> → 중첩 자식의 텍스트를 모음 (속성 @_ 제외)
    const parts = [];
...
export async function fetchAndParseRss(source) {
  // HTML 스크래퍼 경로 (공식 RSS 미제공 소스용)
  if (source.type === 'scraper') {
    if (source.scraper === 'hira_press') return fetchHiraPress(source);
